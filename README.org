...
*** Installation
   ...
   - Recommended : Edit the .htaccess to protect (restrict access to) the back-office
     - Use [[http://aspirine.org/htpasswd_en.htmld][ a trusted online htpassword generator]] (You can also create user/password pair(s) directly on the server with the command "htpasswd -b .access username password")
